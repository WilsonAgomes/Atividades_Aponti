...
[Unidade 2 — Consultas SQL](../atividade_unidade_2_prf/README.md) e
...
- `logs/resumo_final.csv` — resumo numérico da preparação (linhas, colunas, duplicidades, taxa de
  acidentes fatais e total de vítimas).
...
analítica, 19 na base modelável, 5.210 acidentes fatais (taxa global de ~7,18%) e 89.593 vítimas na base analítica. Detalhes completos em
...
## Indicador nas saídas

`total_vitimas` está calculado em `dados_tratados/base_analitica_prf_2025.csv`, descrito no dicionário e consolidado em `logs/resumo_final.csv`. Ele permanece excluído de `base_modelavel_prf_2025.csv`, pois contém o desfecho do acidente e causaria vazamento de informação na modelagem. As bases e os logs foram regenerados pela execução completa do notebook.
