...
SELECT
    *,
    ...
    COALESCE(CAST(mortos AS INTEGER), 0)
      + COALESCE(CAST(feridos_leves AS INTEGER), 0)
      + COALESCE(CAST(feridos_graves AS INTEGER), 0) AS total_vitimas
 FROM prf_2025;

  -- 6. Calculando as métricas gerais: total de acidentes, total de vítimas, total de fatais e o % de acidentes fatais
 SELECT
